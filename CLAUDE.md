...
| **Entrar com Google** (login social) | `docs/specs/login-google.md` (ponteiro) · `docs/tasks/login-google-todo.md` | 🚧 **Fases A–E na `develop`** — **`main` não tem nada**. Botão GIS em `/login`/`/register`, `loginWithGoogle` não persiste, `/register` bloqueia conta existente, toast de auto-ligação, `DeleteAccountModal` reautentica por Google, aviso "Defina uma senha" no Perfil, CSP do GIS no `vercel.json` (Fase D). BUG-E1 corrigido + Checkpoint E verificado (2026-09-10). `VITE_GOOGLE_CLIENT_ID` na Vercel **feita** (2026-09-14). Falta: publicar o app OAuth (só depois da `main` no ar) + verificação pós-deploy da CSP. Plano-mestre: `oratio-api/docs/specs/login-google.md`. |
| **Boas-vindas** (guia de primeira entrada) | `docs/specs/boas-vindas.md` (ponteiro) | 🚧 **Na `develop`** (2026-09-10) — **`main` não tem nada**. Rota `/oratio/boas-vindas` tela cheia, `WelcomeGuide` (3 páginas), `WelcomeGate` no shell redireciona por `showWelcome` do `GET /users/me` (`ARCHITECTURE.md` §3). Coluna + backfill **aplicados em produção** (2026-09-14). Falta QA na tela + promoção pra `main`. Mestre: `oratio-api/docs/specs/boas-vindas.md`. |
...
| **Consentimento de privacidade (LGPD)** (Termos de Uso + Política) | `docs/specs/consentimento-privacidade.md` · `docs/tasks/consentimento-legal-todo.md` | 🚧 **Na `develop`** (merge `ba706e0`, 2026-09-11) — **`main` não tem nada**. `LegalTermsGate` no shell → `/oratio/consentimento` quando `legalTermsAccepted !== true`; `LegalConsentGate` (duas caixas desmarcadas, modos `pre-account`/`post-account`); rotas públicas `/termos-de-uso` e `/politica-de-privacidade`. Texto em `docs/legal/2026-09-11-*.md` + componentes de conteúdo — a Política §8 foi corrigida em-place em 2026-09-14 (retenção real dos registros de acesso). Colunas aplicadas em produção sem backfill. Falta: `/review-pr` (item 6 do checklist) + QA na tela + promoção pra `main`. Mestre: `oratio-api/docs/specs/consentimento-privacidade.md`. |
