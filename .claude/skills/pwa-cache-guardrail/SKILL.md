...
| `CACHE_NAME` (`"oratio-cache-v24"`) | `public/sw.js:1` | cache de assets do service worker | prefixo: caches `oratio-cache-*` antigos são apagados no `activate` |
...
**Os dois números não são a mesma coisa e não sobem juntos automaticamente.** `v10` e `v24` já
