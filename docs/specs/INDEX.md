...
| Entrar com Google | `specs/login-google.md` (ponteiro) | `tasks/login-google-todo.md` | — | `oratio-api/docs/specs/login-google.md` (mestre) | 🚧 **Fases A–E na `develop`** (Fase D CSP mergeada `946cfac`; Fase E 2026-09-09: `loginWithGoogle` não persiste, `/register` bloqueia conta existente, toast de auto-ligação, `disabled` no botão, exclusão de conta só-Google, aviso "Defina uma senha" no Perfil). **✅ BUG-E1 corrigido** (balão "Defina uma senha" via `<Portal/>`, commit `23b0d78`, na `develop` pelo merge `4401808`) **e verificado nas 3 configurações no Checkpoint E (2026-09-10, os 7 cenários OK).** **Sem bloqueio de código pra `main`** — env vars no Render + Vercel **feitas** (2026-09-14); falta: publicar o app OAuth (depois da `main` no ar) + verificação pós-deploy da CSP + smoke iPhone PWA. |
| Boas-vindas (guia de primeira entrada) | `specs/boas-vindas.md` (ponteiro) | — *(sem plano: mudança pequena)* | — | `oratio-api/docs/specs/boas-vindas.md` (mestre) | 🚧 **na `develop` dos dois repos** (2026-09-10) — **`main` não tem nada**. Guia de 3 páginas na 1ª entrada (senha e Google), 1x só. Rota `/oratio/boas-vindas` tela cheia (fora de bottom nav), `WelcomeGuide` (3 páginas, progresso, sem pular/sair) — **animado desde 2026-09-10**: texto que se digita (`useTypewriter.ts`, espelha o Vox), transição real entre páginas, ícone com movimento próprio, tocar completa o texto e arrastar avança; `prefers-reduced-motion` = texto inteiro e instantâneo, camada da página que sai nem monta. **Cada tela é um capítulo (2026-09-10):** título + 1 linha de introdução + lista de 3–4 recursos (ícone `lucide` + nome + meia linha), tudo do app e com rota — Oração diária / Caminhos / Estudo e conversa; a lista é `<ul>/<li>` inteira no DOM desde o início, com cópia `.srOnly`; cabe em 375×667 sem rolar. `WelcomeGate` no shell redireciona por `showWelcome` do `GET /users/me` — **reavalia a cada troca de rota** (2026-09-10: cadastro por senha caía na Home porque o efeito só rodava no boot), o `checked` ref só é marcado depois dos termos aceitos (2026-09-14: o consentimento tem precedência — enquanto `legalTermsAccepted !== true` o `WelcomeGate` não redireciona nem se gasta). `welcomeService.markWelcomeSeen` → `POST /users/me/welcome-seen`. Sem `localStorage`, sem rota nova de auth. 11 testes (3 `WelcomeGate` + 8 `WelcomeGuide`, com os itens de cada capítulo). **Backend na `develop` do `oratio-api`** (`welcomeSeenAt` + `showWelcome` + rota idempotente). Coluna + backfill **aplicados em produção** (2026-09-14, `oratio-api/prisma/db-scripts/2026-09-10-boas-vindas.sql`). Falta o teste manual na tela + promoção pra `main`. |
| Admin: método de entrada | `specs/admin-provedor.md` (ponteiro) | — *(sem plano: mudança pequena)* | — | `oratio-api/docs/specs/admin-provedor.md` (mestre) | 🚧 **na `develop` dos dois repos** (2026-09-10) — **`main` não tem nada**. Ver/filtrar cada usuário no painel admin por Oratio (e-mail+senha) × Google × ambos, **só leitura**. `adminService`: `AdminFilters.provider` + `hasPassword`/`authProviders` em `AdminUser`. `AdminFilterSheet`: grupo de chips "Entrada" (Todos / Só Oratio / Só Google / Ambos). `AdminPanel`: `filterProvider` no mesmo desenho de `filterRole` (contagem, summary, limpar); ícones `Mail`/`Chrome` na listagem e no modal + `AlertTriangle` pra conta sem método (anomalia). **Backend** (`oratio-api` develop): `select` + `where` novos em `getAllUsers`/`getUserDetail`, hash descartado. Sem schema, sem `db push`, sem rota/dependência nova. Falta teste manual no painel + promoção pra `main`. |
...
| Consentimento de privacidade (LGPD) | `specs/consentimento-privacidade.md` | — | `tasks/consentimento-legal-todo.md` | `oratio-api/docs/specs/consentimento-privacidade.md` (mestre) | 🚧 **na `develop` dos dois repos** (merge `ba706e0`, 2026-09-11) — **`main` não tem nada**. Colunas aplicadas em produção sem backfill (2026-09-14). Política §8 corrigida em-place na versão `2026-09-11` (retenção real dos registros de acesso, `ad12c93`, 2026-09-14). Falta: `/review-pr` (item 6 do checklist, nunca rodado) + QA na tela + promoção pra `main`. Desenho original: Mecanismo de consentimento a **dois** documentos (Termos de Uso + Política de Privacidade, aceitos juntos): 4 portas (cadastro por senha, cadastro por Google, login por Google, já logado — `LegalTermsGate`), tela reusável `LegalConsentGate` (duas caixas independentes, ambas desmarcadas por padrão, ambas obrigatórias — modelo Google, sem pré-marcação), rotas novas `/termos-de-uso` e `/politica-de-privacidade` (públicas) e `/oratio/consentimento` (protegida), reúso do `DeleteAccountModal`. **Texto já existe e já foi aprovado** — `docs/legal/2026-09-11-{termos-de-uso,politica-de-privacidade}.md`, não é mais placeholder. Correção desta revisão: o app não guarda "intenções de oração" (`Prayer`/`GeneralPrayer` são catálogo sem `userId`) — o dado sensível real é `Message.content`, `BibleMark.note`, `QuaresmaMichaelPenance.content`, registros de prática devocional e a própria existência da conta. Backend: `User.legalTermsAcceptedAt`/`legalTermsVersion` (renomeado de `privacy*`, um par cobre os dois documentos), `CreateUserDto.legalTermsAccepted` obrigatório, `POST /users/me/legal-terms-accepted`. **Bloqueada por** `oratio-api/docs/specs/vox-protocolo-crise.md` (ver "Dívidas conhecidas") antes de publicar os Termos. |
...
- **Protocolo de crise do Vox — instrução feita, comportamento não provado.** Os Termos de Uso
  (`docs/legal/2026-09-11-termos-de-uso.md` §4) prometem que o Vox encaminha quem está em
  crise/ideação suicida para CVV 188/`cvv.org.br`/SAMU 192. O bloco `## Crise aguda` já está em
  `VOX_IDENTITY` na `develop` do `oratio-api` (`fc6f2e9`, texto aceito por Lucas); os 6 critérios
  de `oratio-api/docs/specs/vox-protocolo-crise.md` seguem abertos até uma chamada real ao modelo.
- ~~**`ARCHITECTURE.md` deste repo ainda cita `guestAllowedPrefixes`**~~ ✅ Resolvido em
  2026-09-14 (`/docs-sync`): §3 e §7 descrevem o roteamento declarativo real (rota fora de
  `<ProtectedRoute>` + `isLoggedIn()`/`GuestGateModal`); o `ac-reviewer` também parou de pedir a
  lista.
- **Dívidas que a Política de Privacidade declarava como fato** (achado ao escrever
  `consentimento-privacidade.md`, 2026-09-11) — detalhe no `oratio-api/docs/specs/INDEX.md`:
  - ~~Remover `RefreshSession.location`~~ ✅ 2026-09-14: a Política §8 passou a declarar a
    localização aproximada e a finalidade dela; a coluna fica.
  - ~~Job de limpeza de sessões expiradas~~ ✅ 2026-09-14: resolvido reescrevendo a frase da §8
