...
| Entrar com Google | `specs/login-google.md` (ponteiro) | `tasks/login-google-todo.md` | — | `oratio-api/docs/specs/login-google.md` (mestre) | 🚧 **Fases A–E na `develop`** (Fase D CSP mergeada `946cfac`; Fase E 2026-09-09: `loginWithGoogle` não persiste, `/register` bloqueia conta existente, toast de auto-ligação, `disabled` no botão, exclusão de conta só-Google, aviso "Defina uma senha" no Perfil). **⚠️ NÃO pronta pra `main`: BUG-E1 de CSS em aberto** — balão do aviso "Defina uma senha" recortado no desktop (`.pwdHint` absoluto dentro de `.profileHero` com `overflow:hidden`; correção proposta: mover pro `<Portal/>`. Ver `oratio-api/docs/specs/login-google.md` → "Fase E → Bugs conhecidos" e `docs/ARCHITECTURE.md`). Falta: teste manual no navegador + verificação pós-deploy da CSP. |
| Prova de identidade (reautenticação p/ operações sensíveis) | `specs/prova-identidade.md` (ponteiro) | *(a criar)* | — | `oratio-api/docs/specs/prova-identidade.md` (mestre) | 📝 **rascunho** — spawned do teste da Fase E. O app trata a senha como prova única → (1) não dá pra excluir conta com o Google se você tem senha (LGPD), (2) não há "esqueci minha senha atual" dentro do app. Frontend: `DeleteAccountModal` com 3 modos + link "Não lembro minha senha" em "Trocar senha". |
