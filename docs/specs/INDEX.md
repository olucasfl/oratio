...
- **Protocolo de crise do Vox ausente — bloqueante da publicação dos Termos de Uso.** Os Termos
  de Uso já aprovados (`docs/legal/2026-09-11-termos-de-uso.md` §4) prometem que o Vox encaminha
  quem está em crise/ideação suicida para CVV 188/`cvv.org.br`/SAMU 192; `vox.prompt.ts`
  (`oratio-api`) não tem nenhuma instrução nesse sentido hoje. Spec própria:
  `oratio-api/docs/specs/vox-protocolo-crise.md`. Exige preflight `doutrina-guardrail` e texto
  aprovado por Lucas antes do commit (`RULES.md` §3) — não é implementação livre.
- **`ARCHITECTURE.md` deste repo ainda cita `guestAllowedPrefixes`** (§3/§7/quirks) como se fosse
  uma lista real no código — não é (só o `RULES.md` foi corrigido, achado ao escrever a spec
  `consentimento-privacidade.md`). Dívida pequena, sem dono ainda.
- **Dívidas que a Política de Privacidade já aprovada declara como fato, mas ainda não são
  verdade** (achado ao escrever `consentimento-privacidade.md`, 2026-09-11) — ver detalhe no
  `oratio-api/docs/specs/INDEX.md` (as três primeiras são mudanças de backend):
  - Remover `RefreshSession.location` (derivável do IP, sem finalidade declarada).
  - Job de limpeza de sessões expiradas — hoje não existe (os `deleteMany` de
    `auth.service.ts` são logout/revogação, não expiração).
